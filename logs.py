...
def rename_log_file(now: datetime) -> None:
    ...
    os.rename(DATA_LOG_PATH, DATA_LOG_PATH[:-4] + '_' + previous_day + DATA_LOG_PATH[len(DATA_LOG_PATH) - 4:])
    os.rename(EVENT_LOG_PATH, EVENT_LOG_PATH[:-4] + '_' + previous_day + EVENT_LOG_PATH[len(DATA_LOG_PATH) - 4:])
